...
> **提示**：编译需要系统安装 `build-essential`、`zlib1g-dev`、`libssl-dev` 等依赖包。
...
# 微信测试（需要先扫码登录）
python3 cli/test_weixin.py login
```
...
**方式 3：开发模式**
```bash
python3 main.py --reload  # 代码变动自动重启
```
...
# 指定时长 120 秒
python3 cli/test_weixin.py receive --duration 120
```

**3. 发送消息**
```bash
python3 cli/test_weixin.py send --to "xxx@im.wechat" --text "你好，世界！"
```
...
sudo yum install zlib-devel openssl-devel bzip2-devel readline-devel sqlite-devel
```
